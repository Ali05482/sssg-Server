...
const apointmentControl = {
  ...
  createAppointMentPlusUser: async (req, res) => {
    ...
    //     req.body.appointmentUniqueId = v4();
    //     const newAppintment = await models.appiontment.create(req.body);
    //     if (!newAppintment) {
    ...
    //       req?.body?.password
    //     );
    //     return res.json({ status: true, data: { user, newAppintment }, msg: `Appointment is started successfully, AppointmentId:  ${newAppintment?.appointmentUniqueId}` });
      }
      else {
  ...
  updateAppointment: async (req, res) => {
    try {
      ...
      console.log("findDoctor", findDoctor?.meetingId)
